...
class Sample():
    ...
    def __init__(self, sample):
        ...
        self.sample = sample
        result = re.search(r"^((MoHQ-(JG|HM|CM|GC|MU|MR|IQ|XX|CQ)-\w+)-\w+)-\w+-\w+(D|R)(T|N)", sample)
        if not result:
            raise SystemExit(f"Sample {sample} doesn't conform to regex '^((MoHQ-(JG|HM|CM|GC|MU|MR|IQ|XX|CQ)-\w+)-\w+)-\w+-\w+(D|R)(T|N)'")
